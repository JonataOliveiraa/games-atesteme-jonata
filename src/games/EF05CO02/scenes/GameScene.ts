...
export class GameScene extends Phaser.Scene {
  ...
  private drawBackground() {
    ...
    veil.fillStyle(0x0f2547, this.phase.context === 'mapa' ? 0.12 : 0.3)
  ...
  private buildPhase() {
    ...
    this.markerLayer = this.add.container(0, 0).setDepth(10)
  ...
  private buildNodes(defs: GraphNode[], side: 'main' | 'alt') {
    defs.forEach(def => {
      ...
      this.paintRing(ring, C.white, C.slate)

      const icon = this.add.image(0, 0, def.textureKey).setDisplaySize(52, 52)
      ...
      const hit = this.add.zone(def.x, def.y, HIT_R * 2, HIT_R * 2)
        .setCircleDropZone(HIT_R)
  ...
  private paintRing(g: Phaser.GameObjects.Graphics, fill: number, stroke: number, thick = 5) {
    g.clear()
    g.fillStyle(fill, 0.97)
    g.fillCircle(0, 0, NODE_R)
    g.lineStyle(thick, stroke, 1)
    g.strokeCircle(0, 0, NODE_R)
  }
  ...
  private buildEdgeMarker(e: GraphEdge) {
    ...
    if (this.phase.kind === 'representar') {
      const hit = this.add.zone(mx, my, 44, 44)
        .setCircleDropZone(22)
        .setInteractive({ useHandCursor: true })
        .setDepth(16)
      hit.on('pointerdown', () => {
  ...
  private registerPointerHandlers() {
    ...
    this.input.on('pointerup', (pointer: Phaser.Input.Pointer) => {
      ...
      if (this.draggingNode) {
        this.paintRing(this.draggingNode.ring, C.white, C.slate)
        this.draggingNode = null
      }
    })
  }
  ...
  private onNodeDown(view: NodeView) {
    ...
    if (kind === 'representar') {
      this.dragFrom = view
      this.paintRing(view.ring, 0xfff7cc, C.amber, 7)
  ...
  private moveNode(view: NodeView, x: number, y: number) {
    ...
    const hit = view.container.getData('hit') as Phaser.GameObjects.Zone
    hit.setPosition(view.x, view.y)
    this.redrawEdges()
  }
  ...
  private finishEdgeDrag(pointer: Phaser.Input.Pointer) {
    ...
    this.dragFrom = null
    this.elasticLayer.clear()
    this.paintRing(from.ring, C.white, C.slate)
  ...
  private refreshRoute() {
    this.nodeViews.forEach(v => {
      ...
      this.paintRing(v.ring, inPath ? 0xede9fe : C.white, inPath ? C.purple : C.slate, inPath ? 7 : 5)
    })
    this.redrawEdges()
  ...
  private flashNode(view: NodeView, color: number) {
    this.paintRing(view.ring, 0xfee2e2, color, 7)
    this.time.delayedCall(280, () => {
      const inPath = this.routePath.includes(view.def.id)
      this.paintRing(view.ring, inPath ? 0xede9fe : C.white, inPath ? C.purple : C.slate, inPath ? 7 : 5)
    })
  }
      ...
      queue.push({
        ...
        steps: [
          ...
          },
          {
            ...
            shape: 'rect', x: W / 2, y: 400, w: 900, h: 420,
