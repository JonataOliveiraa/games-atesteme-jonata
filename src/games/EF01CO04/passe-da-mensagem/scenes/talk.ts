...
/**
 * O balão e a moldura da tela. A moldura pisca nas BORDAS, onde não há nada
 * para ler — é o que faz o acerto parecer grande sem tirar o olho do meio.
 */
export function createTalk(scene: Phaser.Scene) {
    ...
    function paint(accent: number) {
        const w = Math.max(300, text.width + 68)
        ...
        balloonG.fillStyle(C.ink, 1)
        balloonG.fillTriangle(-24, h / 2 - 4, 24, h / 2 - 4, -2, h / 2 + 26)
        balloonG.fillStyle(C.cream, 1)
        balloonG.fillTriangle(-16, h / 2 - 10, 16, h / 2 - 10, -2, h / 2 + 14)
    }
    ...
    return {
        ...
        async say(value: string, accent = C.warn, y = BALLOON.y) {
            text.setText(value)
            paint(accent)
            balloon.setY(y)
